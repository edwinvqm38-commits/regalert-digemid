...
MAX_CANDIDATOS = 6
MAX_PAGINAS_IDENTIDAD = 12


def candidatos_de_pdf(html: str, base_url: str) -> list[tuple[str, str]]:
...
def evidencia_de_candidato(url: str, anchor_text: str, identidad_objetivo) -> EvidenciaDocumental:
    ...
    try:
        ...
        with fitz.open(stream=io.BytesIO(respuesta.content), filetype="pdf") as doc:
            ...
            textos = []
            for indice in range(min(doc.page_count, MAX_PAGINAS_IDENTIDAD)):
...
def resolver_pdf(html: str, base_url: str, norma: dict):
    ...
    candidatos = candidatos_de_pdf(html, base_url)
    if not candidatos:
    ...
    return resolver_pdf_para_norma(evidencias, identidad), identidad
